# ผลลัพธ์
## ดูขนาด Binary
<img width="829" height="315" alt="476668041-3be2bd32-6943-4ca3-9873-a7e6dbf132eb" src="https://github.com/user-attachments/assets/c538c447-58ee-4a97-b069-2e3f16d19056" />
...
1. **Docker vs Native Setup** : อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system
   ...
   **ตอบ**
   ...
   - Docker ช่วยให้ไม่ต้องติดตั้ง ESP-IDF และ dependency เยอะบนเครื่องจริง แค่ดึง image มาก็พร้อมใช้งาน ทำให้สะดวกกว่าและไม่มีปัญหาเวอร์ชันชนกัน
     
2. **Build Process** : อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary

   **ตอบ**

   - ใน Docker จะ mount โค้ดจากเครื่องเข้ามา แล้วใช้ idf.py build ให้ CMake สร้างไฟล์ Makefile → compile source code → link → ออกมาเป็นไฟล์ .bin สำหรับ flash ลงบอร์ด

3. **CMake Files** : บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?

    **ตอบ**

   - ไฟล์ CMakeLists.txt กำหนดโครงสร้างโปรเจ็กต์ เช่น ไฟล์ไหนต้อง compile, ใช้ library อะไร ใน Docker มันจะถูกเรียกอัตโนมัติเมื่อสั่ง build เพื่อรวมทุก module เป็นโปรเจ็กต์เดียว

4. **Git Ignore** : ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?

    **ตอบ**

   - ไฟล์ .gitignore บอกว่าไฟล์ไหนไม่ต้องส่งขึ้น Git เช่น ไฟล์ build หรือ binary เพื่อให้ repo สะอาดและไม่กินพื้นที่

5. **Container Persistence** : ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?

    **ตอบ**

   - ข้อมูลใน container จะหายเมื่อ restart (เช่น build cache หรือการตั้งค่าใน container) แต่ไฟล์โปรเจ็กต์ที่ mount จากเครื่อง (เช่นในโฟลเดอร์ /workspaces) จะยังอยู่เหมือนเดิม

 6. **Development Workflow** : เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system

    **ตอบ**

    - Docker พัฒนาได้เร็ว ไม่ต้องตั้งค่า environment ทุกครั้ง ใช้ได้เหมือนกันทุกเครื่อง ส่วน native system ต้องตั้งค่าด้วยตัวเอง แต่ build เร็วกว่านิดหน่อยและเข้าถึง hardware ได้ตรงกว่า
